...
from detection2 import perform_detection
...
class AimingPipeline:
    ...
    def apply_smoothing(self, dx: float, dy: float) -> tuple:
        ...
        if len(self.smoothing_buffer) > 1:
            avg_dx = sum(x[0] for x in self.smoothing_buffer) / len(self.smoothing_buffer)
            avg_dy = sum(x[1] for x in self.smoothing_buffer) / len(self.smoothing_buffer)
...
class MasterPipeline:
    ...
    def process_frame(self, frame: np.ndarray, frame_info) -> Dict[str, Any]:
        ...
        if targets and getattr(config, "enableaim", False):
            dx, dy, distance = self.aiming_pipeline.calculate_aim_adjustment(targets, frame_info)
    ...
    def _should_trigger(self, frame: np.ndarray, frame_info) -> bool:
        ...
        try:
            ...
            x2, y2 = min(cx + ROI_SIZE, frame.shape[1]), min(cy + ROI_SIZE, frame.shape[0])
