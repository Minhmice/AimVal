...
100% code from v2.0_Modern/scripts/udp_viewer_2.py - NO MODIFICATIONS
...
class PureUdpViewer:
    """
    Pure UDP Viewer using 100% v2 udp_viewer_2 code
    No GUI, no extra features - just core streaming
    """
    ...
    def __init__(self, host: str = "0.0.0.0", port: int = 8080, rcvbuf_mb: int = 64):
        ...
        if TurboJPEG is not None:
            try:
                ...
                self.use_turbo = True
                log("info", "TurboJPEG enabled (always on)")
            except Exception as e:
                log("warn", f"TurboJPEG init failed: {e}")
                pass
        
        # Metrics tracking (100% from v2)
    ...
    def start(self) -> bool:
        """Start UDP viewer - 100% from v2 logic"""
        if self.started:
            return True
            
        try:
            # Socket setup - 100% from v2
            ...
            # Start receiver thread - 100% from v2
    ...
    def stop(self):
        """Stop UDP viewer - 100% from v2 logic"""
        self.started = False
    ...
    def get_latest_frame(self) -> Optional[np.ndarray]:
        """Get latest frame - 100% v2 decode logic"""
        if not self.started:
            return None
        ...
        # Decode using 100% v2 logic
        if self.use_turbo and self.jpeg_decoder is not None:
            try:
                frame = self.jpeg_decoder.decode(buf)  # BGR by default
            except Exception:
        ...
        if frame is not None:
            # Update metrics - 100% v2 timing logic
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "total_ns": self.tty_total_ns
        }


def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Threaded UDP MJPEG viewer")
    p.add_argument("--port", type=int, default=PORT, help="UDP port to listen on")
...
def main() -> None:
    ...
    udp_port = args.port
    log("info", f"Listening on udp://{HOST}:{udp_port}")
