...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(ctrl, text="Nguồn").pack(side="left", padx=(16, 4))
        self.mode_opt = ctk.CTkOptionMenu(
            ctrl, values=["mjpeg", "opencv", "rtsp", "file"]
        )
        self.mode_opt.set(self.cfg.get("udp", {}).get("mode", "mjpeg"))
        self.mode_opt.pack(side="left", padx=4)
        self.url_entry = ctk.CTkEntry(ctrl, width=280)
        self.url_entry.insert(0, self.cfg.get("udp", {}).get("url", "udp://@:5600"))
        self.url_entry.pack(side="left", padx=4)
        self.port_entry = ctk.CTkEntry(ctrl, width=80)
        self.port_entry.insert(0, str(self.cfg.get("udp", {}).get("mjpeg_port", 8080)))
        self.port_entry.pack(side="left", padx=4)
        ctk.CTkButton(ctrl, text="Áp dụng", command=self._apply_source).pack(
            side="left", padx=6
        )
        # Metrics
    ...
    def _build_tab_trigger(self, parent):
        ...
        self.tdelay = ctk.CTkEntry(frm, width=80)
        self.tdelay.insert(0, str(act.get("trigger_delay_ms", 10)))
        self.tcd = ctk.CTkEntry(frm, width=80)
        self.tcd.insert(0, str(act.get("trigger_cooldown", 0.15)))
        row = ctk.CTkFrame(frm)
        row.pack(fill="x", pady=6)
        ctk.CTkLabel(row, text="Delay ms").pack(side="left", padx=6)
        self.tdelay.pack(side="left", padx=2)
        ctk.CTkLabel(row, text="Cooldown").pack(side="left", padx=6)
        self.tcd.pack(side="left", padx=2)
        # Advanced trigger
        self.tr_mode = ctk.CTkOptionMenu(frm, values=["instant", "burst", "adaptive"])
        self.tr_mode.set(self.ctrl.cfg.get("TRIGGER_MODE", "instant"))
        self.tr_burst_cnt = ctk.CTkEntry(frm, width=80)
        self.tr_burst_cnt.insert(0, str(self.ctrl.cfg.get("TRIGGER_BURST_COUNT", 3)))
        self.tr_burst_delay = ctk.CTkEntry(frm, width=80)
        self.tr_burst_delay.insert(
            0, str(self.ctrl.cfg.get("TRIGGER_BURST_DELAY", 0.05))
        )
        row2 = ctk.CTkFrame(frm)
        row2.pack(fill="x", pady=6)
        ctk.CTkLabel(row2, text="Mode").pack(side="left", padx=6)
        self.tr_mode.pack(side="left", padx=2)
        ctk.CTkLabel(row2, text="Burst count").pack(side="left", padx=6)
        self.tr_burst_cnt.pack(side="left", padx=2)
        ctk.CTkLabel(row2, text="Burst delay").pack(side="left", padx=6)
        self.tr_burst_delay.pack(side="left", padx=2)
    ...
    def _apply_source(self):
        ...
        try:
            ...
            port = 8080
        self.ctrl.set_udp_mode_and_url(mode, url, port)
    ...
    def _build_legacy_tab(self, parent):
        ...
        # Render two-column grid for readability
