...
class ViewerApp(ctk.CTk):
    ...
    def _build_general_tab(self):
        ...
        # UDP Status (read-only - tự động khởi động)
        port_frame = ctk.CTkFrame(self.tab_general)
        port_frame.pack(pady=5, fill="x")
        ctk.CTkLabel(port_frame, text="UDP Port (Auto-start)").pack(side="left", padx=6)
        self.udp_port_entry = ctk.CTkEntry(port_frame, state="readonly")
        self.udp_port_entry.configure(state="normal")
        self.udp_port_entry.insert(0, "8080")
        self.udp_port_entry.configure(state="readonly")
