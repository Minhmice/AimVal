...
class TargetTracker:
    """Optional optical-flow aided target persistence.

    Strategy:
    - Prefer direct detections each frame (nearest to FOV center within range)
    - If a previous target exists but disappears, optionally track it forward
      using Lucas-Kanade optical flow on corner features inside its last contour
    - Drop the target if tracking quality is low or memory is too old
    """
    ...
    def __init__(self, config):
        ...
        self.feature_params = dict(
            maxCorners=100, qualityLevel=0.05, minDistance=7, blockSize=7
        )
    ...
    def update(self, frame, potential_targets, fov_center_x, fov_center_y):
        """Update the tracked target using fresh detections or optical flow."""
        frame_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        best_direct_target = self._get_best_target(
            potential_targets, fov_center_x, fov_center_y
        )
        ...
        if self.current_target and (
            time.time() - self.last_seen_time
            > self.config.get("TARGET_MEMORY_DURATION")
        ):
            self.current_target = None
            self.last_tracked_points = None
